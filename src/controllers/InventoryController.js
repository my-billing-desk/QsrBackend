        ...
        orders.forEach(order => {
            if (order.items) {
                order.items.forEach(orderItem => {
                    ...
                    // 1. Try Variant Recipe first
                    if (orderItem.variantId && orderItem.Variant && orderItem.Variant.Recipe) {
                        recipe = orderItem.Variant.Recipe;
                    }

                    // 2. Fallback to Item Recipe
                    if (!recipe && orderItem.Item && orderItem.Item.Recipe) {
                        recipe = orderItem.Item.Recipe;
                    }
                    ...
                    if (recipe && recipe.RecipeIngredients && recipe.autoConsumption !== false) {
                        recipe.RecipeIngredients.forEach(ing => {
                            ...
                            const totalQty = (ing.quantity / yieldQty) * orderItem.quantity;
